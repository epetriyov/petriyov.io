...
Деплой **только вручную** (Actions → «Deploy (build on VPS)» → Run workflow; push в `main` не деплоит): `.github/workflows/deploy.yml` → tar исходников по SSH в `/opt/petriyov.io` на VPS (rsync не используется — его нет на минимальной Ubuntu) → там `docker build` (теги `latest` + `sha-*`, реестра нет) → `docker compose up -d`; хранятся 5 последних sha-образов для отката. `/opt/petriyov.io` перезаписывается каждым деплоем (выживает только `.env` с опциональным пином `IMAGE_TAG`). Подробности: [docs/DEPLOY.md](docs/DEPLOY.md). Меняя Dockerfile/Caddyfile — прогоните локальный чеклист [docs/DOCKER.md](docs/DOCKER.md) и `caddy validate` для prod-конфига.
