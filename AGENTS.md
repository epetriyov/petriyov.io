...
Деплой **только вручную** (Actions → «Deploy (build in CI)» → Run workflow; push в `main` не деплоит): `.github/workflows/deploy.yml` → docker build **на раннере** → готовый образ по SSH (`docker save | gzip | docker load`, реестра нет) → `docker compose up -d`; хранятся 5 последних sha-образов для отката. **Не переносить сборку на VPS** — там 1 CPU / 1 ГБ RAM / 10 ГБ диска, `npm ci`+`astro build` гарантированно ловят OOM. Порты 80/443 и TLS на VPS держит **общий Traefik** (стек `/home/ai-agent`) — сайт регистрируется labels в compose, портов не публикует; на том же домене живёт вебхук Telegram-бота (`/tg/*`, priority=100) — не занимать эти пути и не трогать чужой роутер. На сервере только `/opt/petriyov.io/docker-compose.yml` (+ опциональный `.env` с пином `IMAGE_TAG`). Подробности: [docs/DEPLOY.md](docs/DEPLOY.md). Меняя Dockerfile/Caddyfile — прогоните локальный чеклист [docs/DOCKER.md](docs/DOCKER.md) и `caddy validate` для prod-конфига.
