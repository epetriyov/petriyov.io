...
Всё. Содержимое (исходники, Dockerfile, docker-compose.yml) приедет само при первом деплое — workflow передаёт туда репозиторий целиком. Руками в этом каталоге ничего не правьте: каждый деплой очищает его и наполняет заново (не тронет только `.env`).
...
1. передаёт исходники в `/opt/petriyov.io` (tar по SSH; каталог пересоздаётся, `.env` не трогается);
